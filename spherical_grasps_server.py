...
# coding: utf-8
"""
spherical_grasps_server.py (adattato)
- Robustified quaternion helper.
- Debug/prod behavior parametric:
  - ~object_pose_subscribe_topic: if set, subscribe to PoseStamped of object centroid and generate grasps.
  - debug_mode / debug_pose / debug_pose_topic options.
- Publishes /sphere_poses, /grasp_poses, /object_marker as before.
"""
import rospy
import numpy as np
import math
...
from geometry_msgs.msg import Pose, PoseStamped, PoseArray, Vector3Stamped, Vector3, Quaternion, Point, Vector3
...
# Robust conversion helper: accept Point-like or list/tuple
def _to_list_vec(v):
    if hasattr(v, 'x') and hasattr(v, 'y') and hasattr(v, 'z'):
        return [v.x, v.y, v.z]
    return list(v)

# quaternion from two vectors (robust for Point or list)
def quaternion_from_vectors(v0, v1):
    ...
    try:
        ...
        s = 0.0
    if s == 0.0:
        # vectors are anti-parallel or nearly so
        return None
...
if __name__ == '__main__':
    ...
    # --- New debug / auto-subscribe behavior controlled via rosparams ---
    debug_mode = rospy.get_param("~debug_mode", False)
    debug_pose_topic = rospy.get_param("~debug_pose_topic", "")
    debug_pose_list = rospy.get_param("~debug_pose", None)  # [x,y,z]
    debug_pose_frame = rospy.get_param("~debug_pose_frame", "base_footprint")
    debug_rate = float(rospy.get_param("~debug_rate", 1.0))

    # PRODUCTION subscription: topic that publishes the object centroid PoseStamped
    object_pose_subscribe_topic = rospy.get_param("~object_pose_subscribe_topic", "")  # e.g. "/detected_aruco_pose"

    def _object_centroid_cb(msg):
        try:
            rospy.loginfo("spherical_grasps_server: received object centroid on '%s' -> generating grasps", msg.header.frame_id)
            sg.create_grasps_from_object_pose(msg)
        except Exception as e:
            rospy.logerr("Error generating grasps from object centroid msg: %s", str(e))

    if object_pose_subscribe_topic:
        rospy.loginfo("spherical_grasps_server: subscribing to object centroid topic '%s'", object_pose_subscribe_topic)
        rospy.Subscriber(object_pose_subscribe_topic, PoseStamped, _object_centroid_cb, queue_size=1)

    if debug_mode:
        rospy.loginfo("spherical_grasps_server: DEBUG MODE ENABLED")
        if debug_pose_topic:
            rospy.loginfo("Listening for PoseStamped on '%s' to generate debug grasps", debug_pose_topic)
            def _debug_cb(msg):
                try:
                    sg.create_grasps_from_object_pose(msg)
                except Exception as e:
                    rospy.logerr("Error generating grasps from debug topic msg: %s", str(e))
            rospy.Subscriber(debug_pose_topic, PoseStamped, _debug_cb, queue_size=1)
            rospy.spin()
        elif debug_pose_list is not None:
            try:
                x = float(debug_pose_list[0])
                y = float(debug_pose_list[1])
                z = float(debug_pose_list[2])
            except Exception:
                rospy.logerr("Invalid ~debug_pose param. Expected list [x,y,z]. Falling back to default [1.0,0.0,1.0]")
                x, y, z = 1.0, 0.0, 1.0

            ps = PoseStamped()
            ps.header.frame_id = debug_pose_frame
            ps.pose.position.x = x
            ps.pose.position.y = y
            ps.pose.position.z = z
            ps.pose.orientation.w = 1.0

            rate = rospy.Rate(debug_rate)
            rospy.loginfo("Publishing debug/grasp generation at %s Hz for pose (%s,%s,%s) in frame %s", debug_rate, x, y, z, debug_pose_frame)
            while not rospy.is_shutdown():
                try:
                    sg.create_grasps_from_object_pose(ps)
                except Exception as e:
                    rospy.logerr("Error generating grasps in debug loop: %s", str(e))
                rate.sleep()
        else:
            rospy.logwarn("debug_mode True but neither ~debug_pose_topic nor ~debug_pose provided. Continuing: subscriber (if any) remains active.")
            rospy.spin()
    else:
        rospy.loginfo("spherical_grasps_server started in NORMAL mode. Subscriber topic: '%s'", object_pose_subscribe_topic if object_pose_subscribe_topic else "(none)")
        rospy.spin()